package com.car_rental_system.car_Rental_system_be.configuration;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request, @NonNull HttpServletResponse response, @NonNull FilterChain filterChain) throws ServletException, IOException {
    	System.out.println("JWtAuthenticationFilter");
    	   ...
    	   // Bypass JWT filter for send-otp
        if (request.getRequestURI().contains("/api/auth/send-otp")) {
        	System.out.println("JWTAuthenticationFilter--bypassing jwt fielter");
            filterChain.doFilter(request, response);
            return;
        }
    	
        if (request.getRequestURI().contains("/api/auth/validate-otp")) {
        	System.out.println("JWTAuthenticationFilter--bypassing jwt fielter");
            filterChain.doFilter(request, response);
            return;
        }
